...
even know are solvable."  Rigetti (who is not only named "Chad",
but looks like he should be named "Chad") is building a quantum
...
least, we won't accept patches in the reference `%jael` style.
But we'll always accept patches to bring those patches up to
reference.  For the moment, the code will just define the style
by analogy, but it'll eventually spawn an actual document.  
