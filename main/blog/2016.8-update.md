...
We hope we're done with this research stuff.  There is really
nothing more unpleasant than CS research.  But the protocol and
crypto stated (`%ames` and `%jael`) just needed another round of
basic architecture.  This is fortunately now done.  We think.

Also, we have more to say than we expected.  The concept of an
"update" only makes sense when everyone already knows what's
going on.  The first update is just the status quo.

We'll be issuing *four* public letters in the next month or two:
these infrastructure milestones; an application/user-experience
MVP strategy; a documentation/education plan; and an upbeat,
super-motivating corporate PR narrative suitable for television.
...
The first milestone (August 15) was "first inception" (FI), where
we claimed our true and final urbits.

The next milestone is a "continuity candidate" (CC), a release
where it's at least possible that we'll need no further
continuity breaches and/or reinstalls.  CC requires a nontrivial
amount of further engineering work.

Sometime after that comes "final continuity" (FC), where we
explicitly renounce continuity breaches.  CC to FC is a testing
milestone and is satisfied by collective judgment.

The last milestone is "security freeze" (SF), where we declare
the PKI final and issue live keys.
...
Post to our [new forum](https://urbit.org/fora)!  Don't be
afraid, the water's fine!  Actually there are a few glitches,
but it's hosted on Urbit (and also backed up).  For the less
daring, [r/urbit](https://www.reddit.com/r/urbit) is a reliable
old standby.
