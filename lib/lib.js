...
export function buildPageTree(path, ordering = "", content = false) {
  // get metadata from _index.md file in each folder. This is where page ordering (weight) is stored.
  ...
  return {
    ...
    pages: pages
      ...
      // Sort files according to weight. Weight can be a date or number. The choice between the two is set by the 'ordering' param.
...
export function getOpenGrantsCount() {
  ...
  const count = paths
    ...
    // Count non-completed grants
    .reduce((acc, metadata) => {
      if (metadata.data.extra.completed) {
        return acc;
      }
      return acc + 1;
    }, 0);
