...
// export function getGrantsByCategory(category, num) {
//   const paths = fs.readdirSync(directories.grants);
//   const matches = paths
//     // Get only markdown files
//     .filter((path) => parse(path).ext === ".md")
...
//       matter(fs.readFileSync(join(directories.grants, path)), options)
//     )
//     .filter(grant => grant.data.taxonomies.grant_type === category)
//   return matches
// }

export function getSimilarGrants(type, category, num) {
  // const paths = getAllPosts(directories.grants);
  const bestMatches = getAllPosts(
    ...
    .map((wrappedGrant) => {
      // wrappedGrant.grant.date = JSON.stringify(wrappedGrant.grant.date).replace(/\"/g, "");
