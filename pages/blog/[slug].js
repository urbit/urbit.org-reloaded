...
import { getPostBySlug, getAllPosts, getNextPost, getPreviousPost, formatDate } from '../../lib/lib'
	...
	<Container>
		<SingleColumn>
			...
			<section className="flex flex-col layout-wide pt-24">
				...
				<div className='type-ui text-gray mt-16'>{formatDate(new Date(post.date))}</div>
			...
			
			{
				previousPost === null
					...
					: <div className={`cursor-pointer w-1/2 mr-4`} key={previousPost.slug}>
					  <Link href={'/blog/' + previousPost.slug} key={`post-${previousPost.slug}`}>
						<div>
						<h3>Previous Post</h3>
						  <BackgroundImage className="w-full h-80 rounded-lg" src={previousPost.extra.image || ''} />
						  <p className='text-black mt-2'>{previousPost.title || ''}</p>
						  <p className='text-gray'>{formatDate(new Date(previousPost.date))}</p>
						</div>
					  </Link>
					</div>
			}
			{
				nextPost === null
					...
					: <div className={`cursor-pointer w-1/2 ml-4`}>
						  <Link href={'/blog/' + nextPost.slug} key={`post-${nextPost.slug}`}>
							<div>
								<h3>Next Post</h3>
							  <BackgroundImage className="w-full h-80 rounded-lg" src={nextPost.extra.image || ''} />
							  <p className='text-black mt-2'>{nextPost.title || ''}</p>
							  <p className='text-gray'>{formatDate(new Date(nextPost.date))}</p>
							</div>
						  </Link>
						</div>
			}
